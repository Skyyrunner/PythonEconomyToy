...
function drawGraph(selection, src) {
	var svg = selection;
	d3.json(src, function(e, d){
	    ...
	    svg.append("g").attr("class", "y axis").call(yaxis)
	   	...
	   	svg.append("line").attr("class", "verticalline")
		    ...
		    svg.on("mousemove", function(){moveverticalline(this, xscale, d);});
	})
}

function init() {
	// might as well draw graphs here
	var rows = d3.select("#pricelegends")
	  .selectAll("span")
	...
	d3.json('raw/prices.json', function(e, d){
		var xscale = d3.scale.linear()
					   .domain([0, d.food.length])
					   .range([marginleft, w-marginright]);
		var yscale = d3.scale.linear()
...
function moveverticalline(container, xscale, data) {
	...
	for (i in names) {
		var name = names[i];
		d3.select("." + name).text(data[name][datax].toFixed(1));
	}
}